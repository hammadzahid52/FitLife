...
const HorizontalScrollbar = ({ data, isBodyParts }) => {
  return (
    <div className="p-5 w-full relative container mx-auto max-w-screen-xl">
      <ScrollMenu LeftArrow={LeftArrow} RightArrow={RightArrow}>
        {data.map((item) => (
          <div key={item.id || item} title={item.id || item} className="m-5">
            {isBodyParts ? (
              <BodyPartComp item={item} />
            ) : (
...
function LeftArrow() {
  ...
  return (
    <div
      onClick={() => scrollPrev()}
      className="left-arrow left-[calc(50%-50px)]"
    >
...
function RightArrow() {
  ...
  return (
    <div
      onClick={() => scrollNext()}
      className="right-arrow right-[calc(50%-50px)]"
    >
      <svg className="w-10 h-10" viewBox="0 0 20 20">
        <path
          className="fill-indigo-700 dark:fill-indigo-500"
